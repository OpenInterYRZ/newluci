...
export default function MemoryChatCard() {
  return (
    <div className="flex flex-col md:flex-row md:items-end gap-6 md:gap-10">
      ...
      >
        <div className="flex w-full flex-col overflow-hidden rounded-tl-xl bg-bg-0 shadow-xl">
          {/* Header */}
          <div className="flex items-center justify-between px-8 pt-7 pb-5">
            <span className="text-[28px] font-bold text-text-0">
              Memories
            </span>
            <div className="flex items-center gap-2 rounded-full border border-grey-1 bg-bg-0 px-4 py-2">
              <Calendar className="h-3.5 w-3.5 text-grey-4" />
              <span className="text-sm text-text-0">Feb 6, 2026</span>
              <ChevronDown className="h-3.5 w-3.5 text-grey-3" />
            </div>
          </div>
              ...
              return (
                <div key={si} className="flex items-start gap-4">
                  ...
                  <div className="flex shrink-0 items-center gap-3 pt-4">
                    <span className="w-14 text-xs text-grey-3">
                      {slot.time}
                    </span>
                    <div className="h-px w-8 bg-grey-1" />
                  </div>

                  {/* Entry card */}
                  <div className="flex-1 pb-5 bg-bg-0/60 rounded-xl backdrop-blur-md">
                    <div className="flex flex-col gap-2 rounded-xl px-4 py-3">
                      ...
                      {/* Body text */}
                      <p className="text-[13px] leading-relaxed text-grey-6">
                        {slot.entry.body}
                      </p>

                      {/* Embed card (if present) */}
                      {slot.entry.hasEmbed && slot.entry.embed && (
                        <div className="mt-1 flex items-center gap-3 rounded-lg border border-grey-1 bg-bg-0 px-3 py-2.5">
                          <div className="flex h-10 w-14 shrink-0 items-center justify-center rounded bg-grey-0">
                            <ImageIcon className="h-4 w-4 text-grey-2" />
                          </div>
                          <div className="flex flex-col gap-0.5 overflow-hidden">
                            <span className="truncate text-xs font-semibold text-text-0">
                              {slot.entry.embed.title}
                            </span>
                            <span className="truncate text-[11px] text-grey-4">
                      ...
                      {slot.entry.badge && (
                        <div className="flex items-center gap-1.5 pt-0.5">
                          <Lock className="h-3 w-3 text-grey-3" />
