      ...
      <div className="flex gap-3 w-full max-w-[620px]">
        ...
        <div className="flex-1 rounded-2xl bg-white/95 backdrop-blur-md shadow-[0_8px_40px_rgba(0,0,0,0.12)] overflow-hidden">
          ...
          <div className="flex items-center gap-3 px-5 py-4 border-b border-grey-1">
            ...
            <span className="text-[10px] font-medium text-text-3 bg-grey-0 px-2 py-1 rounded-full">
              3 calls
            </span>
          </div>
          ...
          <div className="px-5 py-4 flex flex-col gap-3">
            ...
            {[
              {
                date: "Jun 5",
                text: "Initial call — interested in annual plan, asked for a case study",
                icon: Phone,
              },
              {
                date: "Jun 12",
                text: "Follow-up — team loved the demo, wants API access for trial",
                icon: MessageSquare,
              },
              {
                date: "Jun 19",
                text: "Budget got cut, revisit in Q3. Decision pending VP approval.",
                icon: Phone,
                highlight: true,
              },
            ].map((entry, i) => (
              <div key={i} className="flex gap-3 items-start">
                <div className="flex flex-col items-center gap-1 pt-0.5">
                  <div
                    className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 ${entry.highlight ? "bg-primary/10" : "bg-grey-0"}`}
                  >
                    <entry.icon
                      className={`w-3 h-3 ${entry.highlight ? "text-primary" : "text-text-3"}`}
                    />
                  </div>
                ...
                <div className="flex-1 min-w-0 pb-1">
                  ...
                  <p
                    className={`text-[12px] leading-relaxed mt-0.5 ${entry.highlight ? "text-primary font-medium" : "text-text-1"}`}
                  >
                    {entry.text}
                  </p>
                </div>
  ...
  const columns = [
    {
      ...
      cards: [
        {
          text: "Send updated proposal to Acme",
          source: "Client Call",
          avatar: "JL",
          avatarBg: "bg-blue-100 text-blue-600",
        },
        {
          text: "Research competitor pricing tier",
          source: "Monday Standup",
          avatar: "KW",
    ...
    },
    {
      ...
      cards: [
        {
          text: "Draft Q3 product roadmap",
          source: "Monday Standup",
          avatar: "YZ",
          avatarBg: "bg-purple-100 text-purple-600",
        },
        {
          text: "Fix onboarding email flow",
              ...
              {col.cards.map((card, ci) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div
                      className={`w-5 h-5 rounded-full flex items-center justify-center text-[8px] font-bold ${card.avatarBg}`}
                    >
                      {card.avatar}
                    </div>
                  </div>
      ...
      <div className="w-full max-w-[520px] rounded-2xl bg-white/95 backdrop-blur-md shadow-[0_8px_40px_rgba(0,0,0,0.12)] overflow-hidden">
        ...
        <div className="px-5 py-4 border-b border-grey-1">
          <div className="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-grey-0 border border-grey-1">
            ...
            <span className="text-[12px] text-text-0 font-medium">
              Why did we drop feature X?
            </span>
          </div>
        </div>
          ...
          {[
            {
              ...
              excerpt:
                '"Feature X adds complexity but only 2% of users asked for it. Let\'s cut it and revisit after launch."',
              speaker: "You",
            },
            {
              type: "Telegram",
              ...
              excerpt:
                '"Agreed on dropping feature X. Kevin said engineering effort is 3 weeks — not worth it pre-launch."',
              speaker: "Kevin",
            },
            {
              type: "Document",
              ...
              excerpt:
                "Feature X deprioritized. Revisit post-launch if user demand > 10%. Owner: Kevin.",
              speaker: null,
            },
          ].map((result, i) => (
            <div
              key={i}
              className="rounded-xl bg-grey-0 border border-grey-1 p-4 flex flex-col gap-2"
            >
              <div className="flex items-center gap-2">
                <span
                ...
                <span className="text-[11px] font-medium text-text-0 ml-1 truncate">
                  {result.title}
                </span>
              </div>
